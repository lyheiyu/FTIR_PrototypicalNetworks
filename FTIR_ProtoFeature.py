...
def findSpectrum(spectrum,indexList,num):
    ...
    return FindArray
# def build_conv_extractor(input_shape):
...
def build_feature_extractor(input_shape, filter):
    model = Sequential([
        Reshape((input_shape, 1), input_shape=(input_shape,)),
        Conv1D(filter, 32, activation='relu', padding="same"),
...
# def proto_loss(support_embeddings, query_embeddings, support_labels, num_classes):
#     prototypes = []
...
#     distances = euclidean_distance(tf.expand_dims(query_embeddings, 1), tf.expand_dims(prototypes, 0))
#     return tf.keras.losses.sparse_categorical_crossentropy(tf.zeros_like(query_embeddings[:, 0]), distances)
...
#
#     distances = np.array([euclidean_distance(query, prototypes) for query in query_embeddings])
#
#     log_p_y = tf.nn.log_softmax(-distances, axis=-1).numpy()
#
#     indices = np.arange(query_labels.shape[0]), query_labels
#     log_p_y = log_p_y[indices]
#
#     loss = -np.mean(log_p_y)
#     return tf.convert_to_tensor(loss)
# def train_proto_net(model, support_data, support_labels, query_data, query_labels, num_classes, epochs=10):
...
#     optimizer = tf.keras.optimizers.Adam()
#
#     support_data = tf.convert_to_tensor(support_data)
#     query_data = tf.convert_to_tensor(query_data)
...
#             loss = proto_loss(support_embeddings, query_embeddings, support_labels, num_classes)
#
#         gradients = tape.gradient(loss, model.trainable_variables)
#         optimizer.apply_gradients(zip(gradients, model.trainable_variables))
#
...
def train_proto_net3(model, support_data, support_labels, query_data, query_labels, num_classes, epochs=10):
    optimizer = tf.keras.optimizers.Adam()
    ...
    for epoch in range(epochs):
        with tf.GradientTape() as tape:
            ...
            support_embeddings = model(support_data, training=True)
            query_embeddings = model(query_data, training=True)
            print("Support embeddings:", support_embeddings)  # Debug 输出
            print("Query embeddings:", query_embeddings)      # Debug 输出
            loss = proto_loss(support_embeddings, query_embeddings, support_labels, query_labels, num_classes)
            print("Loss:", loss)  # Debug 输出

        gradients = tape.gradient(loss, model.trainable_variables)
        if any(g is None for g in gradients):
            print("存在一个或多个梯度为 None")
        else:
            optimizer.apply_gradients(zip(gradients, model.trainable_variables))
...
def train_proto_net4(model, support_data, support_labels, query_data, query_labels, epochs=10):
    optimizer = tf.keras.optimizers.Adam()

    support_data = tf.convert_to_tensor(support_data, dtype=tf.float32)
    query_data = tf.convert_to_tensor(query_data, dtype=tf.float32)
    ...
    for epoch in range(epochs):
        with tf.GradientTape() as tape:
            support_embeddings = model(support_data, training=True)
            query_embeddings = model(query_data, training=True)
            loss = proto_loss(support_embeddings, query_embeddings, support_labels, query_labels)

        gradients = tape.gradient(loss, model.trainable_variables)
        if not any(g is None for g in gradients):
            optimizer.apply_gradients(zip(gradients, model.trainable_variables))
        else:
            print("存在一个或多个梯度为 None")
...
def build_feature_extractor5(input_shape, filters):
    model = Sequential([
        ...
        BatchNormalization(),
        Dropout(0.5),
        Conv1D(filters, 32, activation='relu', strides=1, padding='same'),
        BatchNormalization(),
        GlobalAveragePooling1D(),
...
if __name__ == '__main__':
    ...
    x_train2, x_test2, y_train2, y_test2 = train_test_split(secondData, pid4, test_size=0.3, random_state=1)
    ...
    # prototyes = compute_prototypes(x_train1, y_train1, feature_extractor_model)
    # # 加载预训练模型
    modelName='model/build_feature_extractor'+fileName+'_model.h5'
    num_classes = len(np.unique(pid5))
    feature_extractor_model = load_model(modelName)


    support_embeddings = feature_extractor_model.predict(x_train5)
    query_embeddings = feature_extractor_model.predict(x_test5)
    prototypes = compute_prototypes(support_embeddings, y_train5, num_classes)
    predicted_labels = classify(query_embeddings, prototypes)
    ...
    # extrModelName = 'model/build_feature_extractor' + fileName + '_model.h5'
    # model.save(extrModelName)
    # # Evaluate accuracy
    # accuracy = np.mean(predicted_labels == y_test1)
    # print(f'Accuracy: {accuracy}')
   ...
   #  print(pidM4)
    ...
    # print(score)
# Example usage:
# input_shape = 2000  # Length of your input sequence
# num_classes = 6  # Number of classes
# num_support_samples = 5  # Number of support samples per class
# num_query_samples = 15  # Number of query samples per class
#
# # Generate random data for demonstration
# X_support = np.random.rand(num_classes * num_support_samples, input_shape, 1)
# y_support = np.array([[i] * num_support_samples for i in range(num_classes)]).flatten()
# X_query = np.random.rand(num_classes * num_query_samples, input_shape, 1)
# y_query = np.array([[i] * num_query_samples for i in range(num_classes)]).flatten()
#
# model = build_conv_extractor(input_shape)
# train_proto_net(model, X_support, y_support, X_query, y_query, num_classes, epochs=10)