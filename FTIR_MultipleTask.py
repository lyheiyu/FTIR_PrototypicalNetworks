...
#     log_p_y = tf.nn.log_softmax(-distances)
    ...
    return -tf.reduce_mean(tf.reduce_sum(log_p_y * tf.one_hot(query_labels, depth=num_classes), axis=1))
# def proto_loss2(support_embeddings, query_embeddings, support_labels, query_labels):
#     prototypes, unique_labels = compute_prototypes(support_embeddings, support_labels)
#     distances = tf.norm(tf.expand_dims(query_embeddings, 1) - prototypes, axis=2)
#     log_p_y = tf.nn.log_softmax(-distances)
#     indices = tf.argmax(tf.equal(unique_labels[:, None], query_labels[None, :]), axis=0)
#     return -tf.reduce_mean(tf.gather(log_p_y, indices, batch_dims=1))



def build_feature_extractor5(input_shape, filters):
    model = Sequential([
        Reshape((input_shape, 1), input_shape=(input_shape,)),
        Conv1D(filters, 32, activation='relu', padding="same"),
        GlobalAveragePooling1D(),
...
# def build_simple_feature_extractor(input_shape):
...
#     ])
#     return model
def build_simple_feature_extractor2(input_shape):
...
def build_multi_task_model(input_shape, num_tasks, num_classes_per_task):
    inputs = Input(shape=(input_shape,))

    #base_model= build_simple_feature_extractor(input_shape)
    base_model = build_complex_feature_extractor(input_shape)
    #base_model = build_feature_extractor5(input_shape,64)
    shared_features = base_model(inputs)

    task_outputs = []
    for i in range(num_tasks):
        task_output = Dense(num_classes_per_task[i], activation='softmax', name=f'task_{i}_output')(shared_features)
...
#         class_embeddings = embeddings[tf.where(labels == label)[:, 0]]
#         class_prototype = tf.reduce_mean(class_embeddings, axis=0)
#         prototypes.append(class_prototype)
#     prototypes = tf.stack(prototypes)
#     return prototypes, unique_labels
# def build_feature_extractor6(input_shape):
#     initializer = tf.keras.initializers.HeNormal()
#     inputs = Input(shape=(input_shape,))
#     x = Dense(256, activation='relu', kernel_initializer=initializer)(inputs)
#     x = BatchNormalization()(x)
#     x = Dropout(0.5)(x)
#     x = Dense(128, activation='relu', kernel_initializer=initializer)(x)
#     x = BatchNormalization()(x)
#     x = Dropout(0.5)(x)
#     features = Dense(64, activation='relu', kernel_initializer=initializer)(x)
#     return Model(inputs, features)




def paired_batch_generator(support_data, support_labels, query_data, query_labels, batch_size):
    support_gen = batch_generator(support_data, support_labels, batch_size)
    query_gen = batch_generator(query_data, query_labels, batch_size)
    while True:
        try:
            support_batch_data, support_batch_labels = next(support_gen)
            query_batch_data, query_batch_labels = next(query_gen)
            yield (support_batch_data, support_batch_labels), (query_batch_data, query_batch_labels)
...
def build_multi_task_model6(input_shape, num_classes_per_task):
    feature_extractor = build_feature_extractor6(input_shape)
    inputs = Input(shape=(input_shape,))
    shared_features = feature_extractor(inputs)

    outputs = []
    for num_classes in num_classes_per_task:
        task_output = Dense(num_classes, activation='softmax')(shared_features)
        outputs.append(task_output)

    model = Model(inputs=inputs, outputs=outputs)
    return model, feature_extractor
# def proto_loss6(support_embeddings, query_embeddings, support_labels, query_labels, num_classes):
#     support_prototypes = compute_prototypes6(support_embeddings, support_labels, num_classes)
#     distances = tf.norm(tf.expand_dims(query_embeddings, axis=1) - support_prototypes, axis=2)
#     log_p_y = tf.nn.log_softmax(-distances)
#     loss = -tf.reduce_mean(tf.reduce_sum(log_p_y * tf.one_hot(query_labels, depth=num_classes), axis=-1))
#     return loss
...
#     loss = -tf.reduce_mean(tf.reduce_sum(log_p_y * tf.one_hot(query_labels, depth=num_classes), axis=-1))
#     return loss
# def compute_prototypes6(embeddings, labels, num_classes):
#     prototypes = []
#     for c in range(num_classes):
...
#     return prototypes
# def proto_loss6(support_embeddings, query_embeddings, support_labels, query_labels, num_classes):
#     support_prototypes = compute_prototypes6(support_embeddings, support_labels)
#     distances = tf.norm(tf.expand_dims(query_embeddings, axis=1) - support_prototypes, axis=2)
#     log_p_y = tf.nn.log_softmax(-distances)
#     loss = -tf.reduce_mean(tf.reduce_sum(log_p_y * tf.one_hot(query_labels, depth=num_classes), axis=-1))
#     return loss
...
def compute_prototypes6(embeddings, labels):
    embeddings_np = embeddings if isinstance(embeddings, np.ndarray) else np.array(embeddings)
...
# def proto_loss6(support_embeddings, query_embeddings, support_labels, query_labels):
#     support_embeddings_np = support_embeddings.numpy() if tf.is_tensor(support_embeddings) else np.array(
#         support_embeddings)
#     query_embeddings_np = query_embeddings.numpy() if tf.is_tensor(query_embeddings) else np.array(query_embeddings)
#     support_labels_np = support_labels.numpy() if tf.is_tensor(support_labels) else np.array(support_labels)
#     query_labels_np = query_labels.numpy() if tf.is_tensor(query_labels) else np.array(query_labels)
#
#     prototypes, unique_labels = compute_prototypes6(support_embeddings_np, support_labels_np)
#
#     print("Unique labels:", unique_labels)
#     print("Query labels:", query_labels_np)
#
#     if not np.all(np.isin(query_labels_np, unique_labels)):
#         print("Skipping batch: some query labels not in unique labels")
#         return tf.constant(0.0)
#
#     distances = np.linalg.norm(np.expand_dims(query_embeddings_np, axis=1) - prototypes, axis=2)
#     log_p_y = tf.nn.log_softmax(-tf.convert_to_tensor(distances, dtype=tf.float32))
#
#     indices = np.array([np.where(unique_labels == label)[0][0] for label in query_labels_np])
#
#     if np.any(np.isnan(distances)):
#         print("NaN detected in distances")
#     if np.any(np.isnan(log_p_y.numpy())):
#         print("NaN detected in log_p_y")
#     if np.any(np.isnan(indices)):
#         print("NaN detected in indices")
#
#     return -tf.reduce_mean(tf.gather(log_p_y, indices, batch_dims=1))
def proto_loss6(support_embeddings, query_embeddings, support_labels, query_labels):
    support_embeddings_np = support_embeddings if isinstance(support_embeddings,
                                                             np.ndarray) else support_embeddings.numpy()
    query_embeddings_np = query_embeddings if isinstance(query_embeddings, np.ndarray) else query_embeddings.numpy()
    support_labels_np = support_labels if isinstance(support_labels, np.ndarray) else support_labels.numpy()
    query_labels_np = query_labels if isinstance(query_labels, np.ndarray) else query_labels.numpy()
    ...
    indices = [np.where(unique_labels == label)[0][0] for label in query_labels_np]
    indices = tf.convert_to_tensor(indices, dtype=tf.int32)  # 确保 indices 是 int32 类型
...
#     log_p_y = tf.nn.log_softmax(-tf.convert_to_tensor(distances, dtype=tf.float32))
#
#     # 根据 query_labels 找到每个标签在 unique_labels 中的索引
#     indices = np.array([np.where(unique_labels == label)[0][0] for label in query_labels_np])
#
#     if np.any(np.isnan(distances)):
#         print("NaN detected in distances")
...
def train_multi_task_model6(model, feature_extractor, support_data_list, support_labels_list, query_data_list, query_labels_list, num_classes_per_task, epochs=10):
    initial_learning_rate = 0.00005  # 调整学习率
    ...
    for epoch in range(epochs):
        ...
        for task in range(len(support_data_list)):
            support_data = tf.convert_to_tensor(support_data_list[task], dtype=tf.float32)
            query_data = tf.convert_to_tensor(query_data_list[task], dtype=tf.float32)
            support_labels = tf.convert_to_tensor(support_labels_list[task], dtype=tf.int32)
            query_labels = tf.convert_to_tensor(query_labels_list[task], dtype=tf.int32)

            with tf.GradientTape() as tape:
                support_embeddings = feature_extractor(support_data, training=True)
                query_embeddings = feature_extractor(query_data, training=True)
                proto_loss_value = proto_loss6(support_embeddings, query_embeddings, support_labels, query_labels)
                task_outputs = model(support_data, training=True)
                classification_loss = tf.keras.losses.sparse_categorical_crossentropy(support_labels, task_outputs[task])
                loss = proto_loss_value + tf.reduce_mean(classification_loss)
            ...
            if valid_losses > 0:
                gradients = tape.gradient(total_loss / valid_losses, model.trainable_variables)

                # 打印损失和梯度值
                print(f"Epoch {epoch + 1}/{epochs}, Task {task + 1}, Loss: {loss.numpy()}")
                for g in gradients:
                    if g is not None:
        ...
        print(f'Epoch {epoch + 1}/{epochs}, Total Loss: {total_loss.numpy() / valid_losses if valid_losses > 0 else float("inf")}')

        print(f'Task Losses: {[task_loss.numpy() for task_loss in task_losses]}')
...
# def custom_training_loop(model, feature_extractor, support_data_list, support_labels_list, query_data_list,
#                          query_labels_list, num_classes_per_task, epochs=10, batch_size=32):
#     initial_learning_rate = 0.00005  # 调整学习率
#     lr_schedule = ExponentialDecay(
#         initial_learning_rate,
#         decay_steps=1000,
...
#     min_lr = 1e-6  # 最低学习率
#     factor = 0.5  # 每次减少的因子
#     patience = 10  # 在多少个epoch后没有改善则减少学习率
#     wait = 0  # 等待计数器
...
#             query_data = np.array(query_data_list[task], dtype=np.float32)
#             support_labels = np.array(support_labels_list[task], dtype=np.int32)
#             query_labels = np.array(query_labels_list[task], dtype=np.int32)
#
#             support_gen = batch_generator(support_data, support_labels, batch_size)
#             query_gen = batch_generator(query_data, query_labels, batch_size)
#
#             for (support_batch_data, support_batch_labels), (query_batch_data, query_batch_labels) in zip(support_gen,
#                                                                                                           query_gen):
#                 with tf.GradientTape() as tape:
#                     support_embeddings = feature_extractor(support_batch_data, training=True)
#                     query_embeddings = feature_extractor(query_batch_data, training=True)
#                     proto_loss_value = proto_loss6(support_embeddings, query_embeddings, support_batch_labels,
#                                                   query_batch_labels)
#                     task_outputs = model(support_batch_data, training=True)[task]
#                     classification_loss = tf.keras.losses.sparse_categorical_crossentropy(support_batch_labels,
#                                                                                           task_outputs)
#                     loss = proto_loss_value + tf.reduce_mean(classification_loss)
#
#                     if tf.math.is_nan(loss):
...
#
#                 if valid_losses > 0:
#                     gradients = tape.gradient(total_loss / valid_losses,
#                                               model.trainable_variables + feature_extractor.trainable_variables)
#
#                     # 打印损失和梯度值
#                     print(f"Epoch {epoch + 1}/{epochs}, Task {task + 1}, Loss: {loss.numpy()}")
#                     for g in gradients:
#                         if g is not None:
#                             print(
#                                 f"Gradient max value: {tf.reduce_max(g).numpy()}, min value: {tf.reduce_min(g).numpy()}")
#                             if tf.reduce_max(tf.abs(g)) > 1e6:
#                                 print("Gradient explosion detected!")
#                                 return
#
...
#                         zip(gradients, model.trainable_variables + feature_extractor.trainable_variables))
#
#         epoch_loss = total_loss.numpy() / valid_losses if valid_losses > 0 else float("inf")
#         print(f'Epoch {epoch + 1}/{epochs}, Total Loss: {epoch_loss}')
#         print(f'Task Losses: {[task_loss.numpy() for task_loss in task_losses]}')
#
#         # 手动调整学习率
...
def train_feature_extractor(feature_extractor, support_data_list, support_labels_list, num_classes_per_task, epochs=10,
                            batch_size=32):
    initial_learning_rate = 0.00001  # 降低学习率
    lr_schedule = ExponentialDecay(
        initial_learning_rate,
        decay_steps=1000,
        decay_rate=0.96,
        ...
        for task in range(len(support_data_list)):
            ...
            for batch_data, batch_labels in batch_generator(support_data, support_labels, batch_size):
                with tf.GradientTape() as tape:
                    ...
                    task_outputs = Dense(num_classes_per_task[task], activation='softmax')(embeddings)
                ...
                if valid_losses > 0:
                    gradients = tape.gradient(total_loss / valid_losses, feature_extractor.trainable_variables)
...
def custom_training_loop(model, support_data_list, support_labels_list, query_data_list, query_labels_list,
                         num_classes_per_task, epochs=10, batch_size=32):
    ...
    for epoch in range(epochs):
        ...
        for task in range(len(support_data_list)):
            ...
            paired_gen = paired_batch_generator(support_data, support_labels, query_data, query_labels, batch_size)

            for (support_batch_data, support_batch_labels), (query_batch_data, query_batch_labels) in paired_gen:
                with tf.GradientTape() as tape:
                    support_embeddings = feature_extractor(support_batch_data, training=True)
                    query_embeddings = feature_extractor(query_batch_data, training=True)
                    proto_loss_value = proto_loss6(support_embeddings, query_embeddings, support_batch_labels,
...
if __name__ == '__main__':
    ...
    # feature_extractor=get_feature_extractor(model)
    feature_extractor.save('model/multi_task_model_feature_extractor_model3.h5')
    # 评估多任务模型的准确性
    feature_extractor=load_model('model/multi_task_model_feature_extractor_model3.h5')
